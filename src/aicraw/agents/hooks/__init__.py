...
This package provides hook implementations for AicrawAgent that follow
...
Example:
    >>> from aicraw.agents.hooks import BootstrapHook, MemoryCompactionHook
    >>> from pathlib import Path
    >>>
    >>> # Create hooks (they are callables following AgentScope's interface)
    >>> bootstrap = BootstrapHook(Path("~/.aicraw"), language="zh")
